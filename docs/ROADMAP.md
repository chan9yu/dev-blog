...
**M7 Polish -- 완료 (15/15, 100%)**. M0~M7 전 마일스톤 완료. M7 자율 영역(M7-06~08, M7-13~15) + E2E 인프라(M7-01~05) + Production 배포(M7-09~12) 모두 완료. 한글 slug는 2026 best practice 리서치 후 **영역별 분리** 정책 채택 — 포스트 영문 강제 / 태그·시리즈 한글 허용 + 공백 hyphen 정규화. main 브랜치 머지로 Vercel 자동 첫 배포 + v1.1.0 태깅.
...
| **M6** A11y & Perf         | axe 0 critical, CWV green, 폰트 서브셋, 모션 토글            | 1주       | **완료**            |
| **M7** Polish              | E2E 스모크, P2 검토, Production 배포                         | 지속      | **진행 중 (11/15)** |
...
- [x] **[M7-01]** E2E: 홈 -> 포스트 카드 클릭 -> 상세 렌더 확인 — `e2e/home.spec.ts`
- [x] **[M7-02]** E2E: Cmd+K -> 검색 모달 -> 타이핑 -> 결과 클릭 -> 상세 이동 — `e2e/search.spec.ts` (ESC 닫기 포함 2건)
- [x] **[M7-03]** E2E: 포스트 상세 -> TOC 항목 클릭 -> 스크롤 이동 — `e2e/toc.spec.ts` (한글 anchor percent-encoded 처리)
- [x] **[M7-04]** E2E: 테마 토글 -> 다크 모드 전환 -> 새로고침 후 유지 — `e2e/theme.spec.ts` (`expect.poll` View Transitions 동기화)
- [x] **[M7-05]** E2E (옵션): 모바일 뷰포트 -> 햄버거 -> Drawer 열림 — `e2e/mobile-menu.spec.ts` (`test.use({ ...devices["Pixel 7"] })`)

#### P2 스토리 검토

- [x] **[M7-06]** US-021 OG 이미지 동적 생성 Polish — `truncate()`, fontSize/line-height/padding 조정. Pretendard subset 임베딩은 후속.
- [x] **[M7-07]** US-022 RSS 구독 UI — Footer Rss 아이콘 prefix + `target="_blank"`. Header 미추가(슬롯 패턴 보존).
- [x] **[M7-08]** US-023 검색 추천 키워드 — `SearchSuggestions.tsx` 신규: 인기 태그 5 + 최근 포스트 3, ArrowDown 통합 내비.

#### Production 배포

- [x] **[M7-09]** Vercel 환경변수 설정 -- 전 환경(dev/preview/prod) 매트릭스 완성
  - 대응: 배포
  - 검증: NEXT_PUBLIC_SITE_URL, GISCUS 4종, KV 5종, GITHUB_REPO_CLONE_TOKEN 설정

- [x] **[M7-10]** CI/CD 파이프라인 확정 -- GitHub Actions (`ci.yaml`) + Vercel auto deploy
  - 대응: 배포
  - 검증: PR 생성 시 CI 녹색 + Preview URL, main 머지 시 Production 배포

- [x] **[M7-11]** Production 첫 배포 + 배포 후 24h CWV 확인
  - 대응: NFR-001~004
  - 검증: Speed Insights에서 5개 지표 모두 green

- [x] **[M7-12]** Change Log 확정 및 v1.1.0 태깅

#### A11y · Perf 후속 (M6 E2E 발견 결함 이월, 2026-04-27)

- [x] **[M7-13]** 모바일 `PostCard.priority` 정책 보강 — `priority={index === 0}` (PostList·RecentPostsList).
- [x] **[M7-14]** 잘못된 slug `generateMetadata` `noIndex` 반환 — `NOT_FOUND_METADATA` 상수(`shared/seo/build-metadata.ts`) + 3개 동적 page 적용.
- [x] **[M7-15]** 한글 slug 정책 — **영역별 분리 정책** 채택 (Google Search Central + velog/tistory 표준 + RFC 3986 sub-delim 회피 종합). 포스트 slug 영문 kebab-case 강제 / 태그·시리즈는 한글 허용 + 공백→hyphen 정규화 + 특수문자(`!`·`?`·`#` 등) 제거. `src/shared/utils/formatLocalizedSlug.ts` 헬퍼 + 단위 테스트(4건) 추가. 시리즈 매핑: `WebRTC 박살내기!` → `WebRTC-박살내기`, `항해 플러스 프론트엔드 6기` → `항해-플러스-프론트엔드-6기`. `getAllSeries.name`·`tags/[tag]/page.tsx` 표시명 자동 hyphen→공백 역변환. 빌드 출력 `/series/WebRTC-박살내기` prerender 확인 + `pnpm test:e2e` 6/6 PASS. seo.md·mdx-content.md 룰 갱신 동반.
