...
**M0 Foundation -- 진행 중**. Next.js 16 프로젝트 초기 셋업 완료. `src/app/layout.tsx`, `src/app/page.tsx` 존재. ESLint/Prettier/Lefthook 설정 완료. 디자인 토큰, 공통 레이아웃, 라우팅 쉘 구축이 남아 있다.
...
| -------------------------- | ------------------------------------------------------------ | --------- | ----------- |
| **M0** Foundation          | 빌드 쉘, 디자인 토큰, 공통 레이아웃, 전 라우트 빈 페이지     | 1주       | **진행 중** |
| **M1** UI Skeleton         | 더미 데이터로 전 페이지 UI 완성                              | 2~3주     | 대기        |
| **M2** Content Pipeline    | Submodule, MDX 파서, 읽기 시간, 실데이터 교체, TDD 시작      | 1~2주     | 대기        |
| **M3** Feature Wiring      | Search, Views KV, Comments, Theme persistence, Lightbox 연결 | 2주       | 대기        |
| **M4** Hubs & Aggregations | 태그/시리즈 집계, Popular 빌드 타임 스냅샷, 관련 포스트      | 1~2주     | 대기        |
| **M5** SEO & Syndication   | Metadata, JSON-LD, OG, Sitemap, RSS, Robots                  | 1주       | 대기        |
