...
│
├── skills/              15개 스킬 (6 기존 + 6 신규 + 보조 SEO 3 신규)
...
│   └── 보조 SEO:        ai-seo, programmatic-seo, seo-audit (참고 자료, dev-blog
│                        baseline은 .claude/rules/seo.md 단일 진실 공급원 유지)
│
├── rules/               17개 규칙 (a11y, autonomy, comments, components, icons, mdx-content, no-fallback, project-structure, react, review-discipline, seo, shadcn, styling, testing, theme, typescript, workflow)
│   ├── 아키텍처:        project-structure, components, react, typescript
│   ├── 스타일·테마:     styling, tailwind, theme, shadcn
│   ├── 품질:            a11y, testing, workflow, no-fallback
│   └── 하네스 신규:     mdx-content, seo, autonomy, review-discipline
│
...
| vercel              | 배포 상태·빌드/런타임 로그 조회, Vercel 공식 문서 검색 (배포 명령은 승인 필수) |
...
- **SEO 빌드 게이트** (2026-05-04): `scripts/validate-frontmatter-seo.mjs`가 description 120~160자 + slug 형식을 자동 차단. 외부 인프라(GSC·Bing·Naver 등록·크로스 포스팅)는 `docs/SEO_EXTERNAL.md` 가이드.
...
- vercel MCP **읽기 전용** 호출 (배포 상태·빌드/런타임 로그·Vercel 문서 검색). `deploy_to_vercel` 등 배포 쓰기 명령은 자동 호출 금지 — 사용자 승인 필수
...
2. **`.claude/rules/*.md`** — 17개 코딩 규약
3. **`.claude/skills/*/SKILL.md`** — 15개 스킬 (오케스트레이션·도메인 작업·참조 지식·보조 SEO)
...
- **2026-05-04 (오전)**: SEO 인프라 실험 사이클 — robots AI 크롤러 명시·llms.txt·JSON-LD 8종·FAQ/HowTo schema·SEO 보조 스킬 통합 시도.
- **2026-05-04 (오후)**: 위 실험 항목 중 효과 미검증·Google rich result 자격 축소된 schema 제거 (FAQPage/HowTo/Organization/CreativeWorkSeries/llms.txt/AI 크롤러 명시). **유지된 핵심**: WebSite·Person·BlogPosting·BreadcrumbList JSON-LD 4종, frontmatter 빌드 게이트 (`scripts/validate-frontmatter-seo.mjs`), 콘텐츠 description 정합화, `docs/SEO_EXTERNAL.md` 외부 가이드. 결정 근거: `.claude/rules/seo.md` "회고 — 도입 보류한 항목" 섹션 + `CHANGELOG.md` Unreleased 참조.
- **2026-05-05 ~ 2026-05-07: Post-v1.1.0 Release 사이클 정착** (M7 완료 + production 운영 단계). M0~M7 마일스톤 단위가 아닌 **hotfix release** 패턴으로 운영 (사용자 검수 → 즉시 정정). v1.1.5(SEO 정합성) → v1.1.6(Footer fixed badge) → v1.1.7(UX 정합성 5건 — 모바일 hide·기본 grid·lightbox 슬라이드/적응형). PR `develop → main` + `merge commit` (squash 금지) + `v*.*.*` annotated tag → `release.yaml` 자동 발행. 사이클별 산출물은 `docs/TASKS.md` "Post-v1.1.0 Release Cycle" 표 참조.
- **2026-05-07: GSC SEO Audit + Ralph QA 외부 검증 패턴 도입**. release 후 (1) Google Search Console에서 sitemap/색인/탐색경로/메시지 진단 (`_workspace/verification/gsc_seo_audit_*.md`), (2) Playwright MCP로 모든 라우트 + 인터랙션 시나리오 E2E (`_workspace/verification/ralph_qa_*_cycle_*.md`). v1.1.5 cycle 1에서 BLOCKER 1건(Footer 위치 deploy gap) + MEDIUM 1건(canonical 도메인) 발견 → 즉시 v1.1.6/v1.1.7 hotfix로 해소.
- **2026-05-07: workflow.md 룰 격상** (memory → 룰). 두 신규 섹션 추가:
  - `## ⛔ Squash merge 금지` — PR #31~#34 모두 squash 머지로 main이 develop의 graph ancestor 끊김 → 매 PR 마다 conflict 폭주. 사용자 명시 짜증 후 룰 격상.
  - `## PR 생성 전 base 브랜치 동기화 필수` — `develop → main` PR 생성 직전 `git fetch origin main && git merge origin/main` 의무화.
  - 두 룰은 conversation memory(`feedback_no_squash_merge`·`feedback_pr_main_sync`)에서 출발하여 영구 enforce 위해 룰 파일로 격상됨. **자가진화 사례**.
