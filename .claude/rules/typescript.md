...
- 배럴 파일(index.ts): **서버/클라이언트 경계를 고려한 정책** (상세: `project-structure.md` §배럴 정책)
  ...
  - **배럴은 re-export만 허용**: `export { X } from "./X"` / `export type { T } from "./T"` 형태만 가능. 타입·상수·함수 직접 정의 금지 (`types/index.ts` 예외 없음). 정의는 동료 파일(`SearchResult.ts`·`routes.ts` 등)에 두고 배럴은 경계 선언만 담당. JSDoc 블록은 허용하되 간결 유지.
