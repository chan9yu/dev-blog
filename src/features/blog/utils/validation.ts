...
export function validateSlugConsistency(filename: string, urlSlug: string): void {
	...
	if (fileSlug !== urlSlug) {
		throw new Error(`[${filename}] 파일명과 slug 불일치: ${fileSlug} !== ${urlSlug}`);
	}
}

/**
 * 시리즈 seriesOrder 중복 검증
 * @throws {Error} seriesOrder 중복 시 에러 발생
 */
export function validateSeriesIndex(series: SeriesBucket): void {
	...
	if (duplicates.length > 0) {
		throw new Error(`[시리즈: ${series.name}] seriesOrder 중복: ${duplicates.join(", ")}`);
	}
	...
	if (indices.length > 0) {
		...
		if (hasGaps) {
			console.warn(`⚠️  [시리즈: ${series.name}] seriesOrder가 연속적이지 않습니다`);
		}
	}
}
